...
class RpcConnection extends Base {
  constructor(options) {
    ...
    this._sentReqs = new Map(); // req数据包管理池，多事件场景下解决方案。在处理请求/错误时，从中取出
  ...
  _connect() {
    ...
    /**
     * TODO: 为啥这里要关闭这个算法？
     * @refer https://www.ibm.com/developerworks/cn/linux/l-hisock.html
     * -  要发送极小的 heartbeat（心跳包），如果开启该算法就有问题
     * -  可以在上层将数据包根据需要缓存后发送
     */
    this._socket.setNoDelay(this.options.noDelay);
    // socket 连接超时后抛出异常关闭，FIXME: 把 egg-dnode 重构掉
    this._socket.setTimeout(this.options.connectTimeout/* 3000 */, () => {
      const err = new Error('socket#' + this.url + ' connect timeout(' + this.options.connectTimeout + 'ms)');
      err.name = 'RpcSocketConnectTimeoutError';
      this.close(err);
      this.ready(err);
    });
    // 注意：这里的事件绑定，使用的都是 `once`
    ...
    if (this.options.classMap) opts.classMap = this.options.classMap;
    if (this.options.proto) opts.proto = this.options.proto;
  ...
  /**
   * FIXME: egg-dnode 需要加上这个功能，要修改协议
   * 心跳，用户维护长连接
   */
  heartbeat() {
  ...
  async invoke(req) {
    ...
    this._sentReqs.set(id, { req, resPromise, timer }); // 存档
