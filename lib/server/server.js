...
const defaultOptions = {
  ...
  codecType: 'hessian2',
  classCacheClass: Map, // 这个 cache 为何要做成可配置的？
};

class RpcServer extends Base {
  /**
   * Rpc 服务提供方
   * Server：服务提供方整体，负责管理 service
   * Service：服务对象，可以对一组相同属性的方法进行分组。在 应用启动时 addService 创建
   * server/listenPort：Server可以在多个端口启动服务/
   * Connection：和客户端的连接实例，每个 server 一般要维护多个 connection。在 Server 响应 Socket 时创建
   * Response：响应数据模型，在 connection 响应请求时，从本地已经注册的service缓存中，取出对应的 service，进行响应
   * 是否还需要结合cluster，集群启动？
   * rpcServer 的中间件好像没什么用？！
  ...
  async _handleRequest(req, conn) {
    ...
    try {
      ...
      // 这里处理响应，是通过共享数据引用的方式来实现。
      // 在 service.invoke 时修改res的内容，在 finally 中发送回去，就是已经处理完毕的res
      // 这也是一种很灵活的方法，但是存在不稳定的问题
